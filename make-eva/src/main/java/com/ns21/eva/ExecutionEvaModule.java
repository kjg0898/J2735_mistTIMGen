...
public class ExecutionEvaModule extends AbstractVerticle {
    private static final Logger logger = LoggerFactory.getLogger(ExecutionEvaModule.class);


    private void processFile(String filePath, Function<String, String> lineProcessor) throws IOException {
        String resourcePath = "make-eva/src/main/resources/" + filePath;
        int i = 0;
    ...
    public void start(Promise<Void> startPromise) {
        // 파일 처리를 위한 비동기 작업 생성
        Future<Void> processJson = processFileAsync("testEvaJson.txt", data -> {
            try {
        ...
        Future<Void> processJ2735 = processFileAsync("testEvaJ2735.txt", data -> {
            try {
    ...
    private Future<Void> processFileAsync(String filePath, Function<String, String> lineProcessor) {
        ...
        vertx.executeBlocking(future -> {
            try {
                BufferedReader reader = new BufferedReader(new FileReader(new File("make-eva/src/main/resources/" + filePath)));
