...
public class ExecutionSpatModule extends AbstractVerticle {
    private static final Logger logger = LoggerFactory.getLogger(ExecutionSpatModule.class);


    private void processFile(String filePath, Function<String, String> lineProcessor) throws IOException {
        String resourcePath = "make-spat/src/main/resources/" + filePath;
        int i = 0;
    ...
    private Future<Void> processFileAsync(String filePath, Function<String, String> lineProcessor) {
        ...
        vertx.executeBlocking(future -> {
            try {
                BufferedReader reader = new BufferedReader(new FileReader(new File("make-spat/src/main/resources/" + filePath)));
